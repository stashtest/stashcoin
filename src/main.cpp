...
uint256 hashGenesisBlock("0x999d98fcb5f14000498d8c60c7ca930cb0a75866c45c3cbefcb8a85f42cb52f5");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Stashcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "The Internet of Money.\n";
...
{
    // Stashcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
// stashcoin: increasing Nfactor gradually
...
{
    int64 qSubsidy = 40 * COIN;
    int64 nSubsidy;
    int difference_ = (nHeight / 50000);

    nSubsidy = qSubsidy - difference_ ;

    return nSubsidy + nFees;



//make sure the reward is at least 5 STA
if(nSubsidy < 5) {
nSubsidy = 5 * COIN;
}
...
static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Stashcoin: 3.5 days
static const int64 nTargetSpacing = 120; // Stashcoin: 2 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Stashcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        ...
        if (fTestNet) {
                if (pindexLast->nHeight+1 >= 2116) { DiffMode = 2; } // stashcoin, 100 blocks after first legacy diff adjustment
        }
        else {         
            if (pindexLast->nHeight+1 >= 26754) { DiffMode = 2; }  //stashcoin, 5 days after 27/01/2014 12:00 UTC
        }
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Stashcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Stashcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x999d98fcb5f14000498d8c60c7ca930cb0a75866c45c3cbefcb8a85f42cb52f5");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Drug lord 'El Chapo' Guzman captured in Mexico 02/22/14";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nNonce = 3367273;
        block.nTime = 1393149295;
        ...
        assert(block.hashMerkleRoot == uint256("0x73440638b6ae2bc9568f0d906f2c4429ae4819652864cd14b59aafc9776a5ce7"));
...
unsigned char pchMessageStart[4] = { 0xfa, 0xbf, 0xb5, 0xda }; // Stashcoin: increase each by adding 1 to bitcoin's value.
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
              ...
              strCommand == "filterclear"))
    {
        ...
        return error("peer %s attempted to set a bloom filter even though we do not adstashise that service",
                     pfrom->addr.ToString().c_str());
    }
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("StashcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("StashcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static StashcoinMiner(CWallet *pwallet)
{
    printf("StashcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("stashcoin-miner");
    ...
    try { loop {
        ...
        printf("Running StashcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
        ...
        loop
        {
            ...
            unsigned long int c_sp_sz_CB = ((1 << (GetNfactor(pblock->nTime) + 1)) * 128 ) + 63;
            
            char scratchpad[c_sp_sz_CB];
    ...
    {
        printf("StashcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&StashcoinMiner, pwallet));
}
